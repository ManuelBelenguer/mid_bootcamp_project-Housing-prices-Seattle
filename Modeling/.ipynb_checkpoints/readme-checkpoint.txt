...
4. On a fourth and final approach we performed a logarithmic transformation for the variables "sqft_living", "sqft_lot" and " price".
    - We still keeped the distance from each property to points of interest. (This might be a not very useful action as it doesn't improves the model, and it will be more cost effective).
        ...
        Results:
              ...
              - With these results we assume that the model cannot be improved. R2 and Adjusted R2 metrics show that our model have a overall good performance. On the other 
              hand, MAE and RMSE show that for some of the houses the error we'll see is going to be high. These cases will refer to outliers. Noe that these outliers we're
              the model doesn't perform so good are not the one that are above the Q3, they're actually bellow Q1 which makes us conclude that our model is not good predicting
